...
export class AppStateService {
  ...
  private ensureDefaultPromise?: Promise<Match>;
  ...
  async ensureDefaultMatch(title: string): Promise<Match> {
    if (!this.ensureDefaultPromise) {
      this.ensureDefaultPromise = (async () => {
        await this.migrateLegacyLocalStore();
        await this.loadMatches();
        const existing = this.getMatches()[0];
        if (existing) {
          if (!this.selectedMatchUuid) {
            this.selectedMatchUuid = existing.uuid;
          }
          return existing;
        }
        const created = await this.createMatch(title);
        this.selectedMatchUuid = created.uuid;
        return created;
      })().finally(() => {
        this.ensureDefaultPromise = undefined;
      });
    }
    const match = await this.ensureDefaultPromise;
    if (!this.selectedMatchUuid) {
